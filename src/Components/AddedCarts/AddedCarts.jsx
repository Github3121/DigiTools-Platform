...
const AddedCarts = ({ cartItems, removeFromCart, clearCart, notify }) => {
    return (
                ...
                <div className='flex flex-col gap-4 mt-10 w-full'>
                    {cartItems.map((item) => (
                        <div key={item.id} className='flex items-center justify-between gap-4 bg-[#F9FAFC] border-b border-[#F6F6F6] py-4 w-full py-[22.5px] px-[20px] rounded-[16px]'>
                            ...
                            <button onClick={() => { removeFromCart(item.id); notify(`${item.name} has been removed from the cart!`); }} className='text-[#FF3980] cursor-pointer active:border active:border-red-500 hover:text-white hover:bg-black py-1.5 px-2 rounded-full duration-500 font-bold'>Remove</button>
                        </div>
                    ))}
                    <div></div>
                    ...
                    <div className='flex justify-between items-center mt-6 w-full'>
                        <button onClick={() => { clearCart(); notify('Checkout Successful! Cart has been cleared!'); }} className='bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white py-[15px] rounded-full hover:opacity-90 transition-opacity duration-300 w-full text-[16px] font-bold cursor-pointer'>Proceed to Checkout</button>
                    </div>
                </div>
            )}
        </div>
